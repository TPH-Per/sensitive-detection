...
class TaskGatedAttentionV6(nn.Module):
    ...
    Feature routing (isolation by design):
      V_pool = [CLIP(768) | Flow(3) | YOLO(1) | Gore(1)] = 773-dim
      S_pool = [CLIP(768) | Flow(3) | Gore(1)]           = 772-dim
    ...
    # Pool dimensions (fixed by feature layout)
    _V_POOL_DIM = 768 + 3 + 1 + 1   # CLIP + Flow + YOLO + Gore = 773
    _S_POOL_DIM = 768 + 3 + 1       # CLIP + Flow + Gore         = 772
    ...
    def __init__(
        ...
        v_pool = clip_dim + 3 + 1 + 1  # CLIP + Flow + YOLO + Gore
        s_pool = clip_dim + 3 + 1      # CLIP + Flow + Gore
    ...
    def forward(
        ...
        S_pool = torch.cat([clip_s, flow_b, gore_b], dim=-1)          # [B, T, 772]
